...
export default function TemperatureMapPage() {
  ...
  const weatherDataRef = useRef(buildPointWeatherFeatureCollections(MAIN_CITIES, {}).temperature);
  ...
  useEffect(() => {
    ...
    if (map.getLayer('temperature-heat')) map.setLayoutProperty('temperature-heat', 'visibility', heatVis);
    if (map.getLayer('temperature-city-points')) map.setLayoutProperty('temperature-city-points', 'visibility', markersVis);
    if (map.getLayer('temperature-labels')) map.setLayoutProperty('temperature-labels', 'visibility', markersVis);
  }, [displayMode]);
  ...
  useEffect(() => {
    ...
    const updateWeatherSource = () => {
      ...
      const source = map.getSource('temperature-cities');
      if (source) {
    ...
    const refreshLiveWeather = async () => {
      ...
      if (isDisposed) return;
      weatherDataRef.current = buildPointWeatherFeatureCollections(MAIN_CITIES, weatherByZone).temperature;
      updateWeatherSource();
    };
    ...
    map.on('load', () => {
      ...
      map.addSource('temperature-cities', {
        type: 'geojson',
    ...
    <div style={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 14px', borderBottom: '1px solid var(--glass-border)', background: 'var(--bg-surface)' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>Live Temperature Heatmap</div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Major cities with live temperature</div>
        </div>
      </div>
